  ...
  return (
    <div className="institution-page">
      <div className="page-title-row">
        <div>
          <p>2026년 8월 12일 수요일</p>
          <h1>안녕하세요, 김담당님 👋</h1>
          <span>오늘도 따뜻한 이웃 돌봄을 함께 시작해 볼까요?</span>
        </div>
      ...
      <section className="metric-grid">
        {data.metrics.map((item) => (
          <article className={`metric-card ${item.tone}`} key={item.key}>
      ...
      <div className="dashboard-grid">
        <section className="panel recent-reports">
          <div className="panel-title">
            <div>
              <h2>최근 접수 제보</h2>
              <p>우선 확인이 필요한 제보입니다.</p>
            </div>
            <Link to="/institution/reports">전체 보기 →</Link>
          </div>
          <div className="report-list">
            {data.recentReports.map((item) => (
              <article key={item.id}>
                ...
                <span
                  className={`table-status ${item.status.replace(" ", "-")}`}
                >
                  {item.status}
                </span>
              </article>
