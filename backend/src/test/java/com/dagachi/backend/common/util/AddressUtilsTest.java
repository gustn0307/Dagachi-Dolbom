...
class AddressUtilsTest {
    ...
    @Test
    @DisplayName("extractRegion - 숫자+가로 끝나는 구시가지 법정동도 동과 동일하게 인식한다")
    ...
    @Test
    @DisplayName("extractRegion - 동/리/읍/면이 없으면 로/길로 끝나는 도로명으로 대체한다")
    ...
    @Test
    @DisplayName("extractRegion - 동/도로명이 모두 없으면 구/군으로 대체한다")
    ...
    @Test
    @DisplayName("extractRegion - 아무 패턴도 매칭되지 않으면 '지역 정보 없음'을 반환한다")
    ...
    @Test
    @DisplayName("calculateAgeGroup - birthYear가 null이면 '연령 정보 없음'을 반환한다")
    void calculateAgeGroup_출생연도가_없으면_기본값을_반환한다() {
        assertThat(AddressUtils.calculateAgeGroup(null)).isEqualTo("연령 정보 없음");
    }

    @Test
    @DisplayName("calculateAgeGroup - 만 59세(60세 미만)는 '50대 이하'다")
    ...
    @Test
    @DisplayName("calculateAgeGroup - 만 60세는 '60대'다 (경계값)")
    ...
    @Test
    @DisplayName("calculateAgeGroup - 만 70세는 '70대'다 (경계값)")
    ...
    @Test
    @DisplayName("calculateAgeGroup - 만 90세 이상은 '90대 이상'이다 (경계값)")
    ...
    @DisplayName("parseAgeGroupBucket - 허용된 라벨은 대표 버킷값으로 변환한다")
    ...
    @DisplayName("parseAgeGroupBucket - 허용되지 않은 라벨은 null을 반환한다")
